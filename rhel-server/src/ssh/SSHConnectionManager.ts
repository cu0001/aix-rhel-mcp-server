...
export class SSHConnectionManager {
  ...
  private createDirectConnection(): Promise<ActiveConnection> {
    logger.info("Connecting directly to RHEL", { host: this.appConfig.rhel.host });
    const config = buildConnectConfig(this.appConfig.rhel);
    ...
    return new Promise((resolve, reject) => {
      ...
      client.on("ready", () => {
        logger.info("SSH connection established", { host: this.appConfig.rhel.host });
        resolve({ client });
      });
      client.on("error", (err) => {
  ...
  private createProxyConnection(): Promise<ActiveConnection> {
    ...
    const rhel = this.appConfig.rhel;
    logger.info("Connecting to RHEL via proxy", { proxy: proxy.host, target: rhel.host });

    return new Promise((resolve, reject) => {
      const proxyClient = new Client();

      proxyClient.on("ready", () => {
        proxyClient.forwardOut("127.0.0.1", 0, rhel.host, rhel.port, (err, stream) => {
          if (err) { proxyClient.end(); reject(new SSHError("Port forward failed", err)); return; }

          const rhelClient = new Client();
          rhelClient.on("ready", () => {
            logger.info("SSH connection established via proxy", { proxy: proxy.host, target: rhel.host });
            resolve({ client: rhelClient, proxyClient });
          });
          rhelClient.on("error", (err) => { proxyClient.end(); reject(err); });
          rhelClient.connect(buildConnectConfig(rhel, stream));
        });
      });
