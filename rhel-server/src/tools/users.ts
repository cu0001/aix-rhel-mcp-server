...
export function createUserTools(ssh: SSHConnectionManager): ToolHandler[] {
  return [
    {
      definition: {
        name: "list_rhel_processes",
        description: "List running processes on the RHEL system",
        inputSchema: {
          type: "object",
          properties: {
    ...
    },
    {
      definition: {
        name: "get_rhel_users",
        description: "Get information about users, groups, login sessions, and sudo configuration on the RHEL system",
      ...
      async execute() {
        const output = await runAll(ssh, [
          ...
          "who",
          "last -n 20",
          "grep -R '^[^#]' /etc/sudoers /etc/sudoers.d 2>/dev/null || true",
        ]);
        return text(output);
      },
    },
    {
      ...
      async execute(args) {
        ...
        if (target === "users" || target === "all") {
          const out = await ssh.execute("getent passwd | head -n 200");
          results.push(`--- Users (Sample) ---\n${out}`);
        }
        if (target === "groups" || target === "all") {
          const out = await ssh.execute("getent group | head -n 200");
