...
export function loadConfig(): AppConfig {
  const host = process.env.AIX_HOST;
  const username = process.env.AIX_USERNAME;

  if (!host || !username) {
    throw new Error("AIX_HOST and AIX_USERNAME environment variables are required");
  }

  const password = process.env.AIX_PASSWORD;
  const privateKeyEnv = process.env.AIX_PRIVATE_KEY;
  
  // If AIX_PRIVATE_KEY looks like a file path, read the file content
  let privateKey: string | undefined;
  if (privateKeyEnv) {
    const expandedPath = expandTilde(privateKeyEnv);
    console.error(`[DEBUG] AIX_PRIVATE_KEY env: ${privateKeyEnv}`);
  ...
  const aix = buildCredentials(
    host,
    parseInt(process.env.AIX_PORT || "22"),
    username,
    password,
    privateKey
  );
  ...
  if (proxyPrivateKeyEnv) {
    ...
    console.error(`[DEBUG] PROXY_PRIVATE_KEY env: ${proxyPrivateKeyEnv}`);
    console.error(`[DEBUG] Expanded path: ${expandedPath}`);
    console.error(`[DEBUG] File exists: ${existsSync(expandedPath)}`);
    if (existsSync(expandedPath)) {
      proxyPrivateKey = readFileSync(expandedPath, 'utf8');
      console.error(`[DEBUG] Proxy private key loaded, length: ${proxyPrivateKey.length}`);
    } else {
      // Assume it's the key content itself
      proxyPrivateKey = proxyPrivateKeyEnv;
      console.error(`[DEBUG] Using proxyPrivateKeyEnv as key content directly`);
    }
  }
